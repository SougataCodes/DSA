class Solution {
    public String shortestCommonSupersequence(String str1, String str2) {
        ...
        // Step 2: Construct SCS from DP Table
        int i = m, j = n;
        StringBuilder scs = new StringBuilder();

        while (i > 0 && j > 0) {
            if (str1.charAt(i - 1) == str2.charAt(j - 1)) { // If same character in both
                scs.append(str1.charAt(i - 1));
                i--;
                j--;
            } else if (dp[i - 1][j] < dp[i][j - 1]) { // If from top (str1 side)
                scs.append(str1.charAt(i - 1));
                i--;
            } else { // If from left (str2 side)
